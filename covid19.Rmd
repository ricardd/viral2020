...
```{r, fig.height=6, fig.width=8, echo=FALSE}
library(dplyr)
my.countries <- c("AT","SE","CZ","IT","ES","DE","FR","CA")
...
country.cases.fct <- function(g.id){
  ...
  initial.num.cases <- this.df[as.numeric(this.df$days.since.100.cases)==1,"cumsum.cases"]
  my.logistic <- nls(cumsum.cases ~ my.logistic.fct(a=initial.num.cases, b,c,as.numeric(days.since.100.cases)), data=this.df, start=list(b=0.34,c=c.init))
