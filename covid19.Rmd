...
df2 <- merge(df, first.day.100, by="countriesAndTerritories")
df2$days.since.100 <- df2$dateRep.x - df2$dateRep.y

n <- df2 %>% filter(dateRep.x == format(Sys.time(),"%Y-%m-%d")) %>%
 top_n(20, cumsum.cases) %>%
 arrange(desc(cumsum.cases))

g<-
df2 %>%
  filter(countriesAndTerritories %in% n$countriesAndTerritories, as.numeric(df2$days.since.100)>0) %>%
...
```{r}
my.logistic.fct <- function(b=0.1, c=1E03, ti){
  a<-100
  out <- c / (1 + (((c-a)/a)*exp(-b*ti)))
  return(out)
}

country.fct <- function(g.id){
  ...
  my.lm <- lm(log(cumsum.cases)~days.since.100, data=this.df)
  pred.lm <- data.frame(dateRep=this.df$dateRep.x, cumsum.cases=exp(fitted(my.lm)))
  
  my.logistic <- nls(cumsum.cases ~ my.logistic.fct(b,c,as.numeric(days.since.100)), data=this.df, start=list(b=0.34,c=c.init))
  logistic.pred <- data.frame(dateRep=this.df$dateRep.x, days.since.100=as.numeric(this.df$days.since.100), cumsum.cases=fitted(my.logistic))
  ## inflection point
  t50 <- (log((coef(my.logistic)[2]-100)/100) - log(1))/coef(my.logistic)[1]
  n50 <- my.logistic.fct(coef(my.logistic)[1], coef(my.logistic)[2], ti=t50)
  ...
  g<-ggplot(this.df, aes(dateRep.x, cumsum.cases)) +
    ...
    geom_point(aes(x=min(dateRep.x)+t50, y=n50)) +
    annotate("text",x=min(this.df$dateRep.x)+(as.numeric(diff(range(this.df$dateRep.x)))*0.4), y=n50, label=logistic.text) +
...
us.logistic <- nls(cumsum.cases ~ my.logistic(a,b,c,as.numeric(days.since.100)), data=usdata, start=list(a=1E03,b=0.28,c=1E06))
...
long.df <- data.frame(days.since.100=0:60)
us.logistic.pred.long <-data.frame(dateRep=seq(min(usdata$dateRep.x),min(usdata$dateRep.x)+60,1), days.since.100=0:60, cumsum.cases=my.logistic(coef(us.logistic)[1], coef(us.logistic)[2], coef(us.logistic)[3], ti=long.df$days.since.100))
...
ggplot(usdata, aes(dateRep.x, cumsum.cases)) +
  ...
  xlim(min(usdata$dateRep.x),min(usdata$dateRep.x)+60) +
  guides(color=guide_legend("Model type")) +
...
canada.logistic <- nls(cumsum.cases ~ my.logistic(a,b,c,as.numeric(days.since.100)), data=canadadata, start=list(a=1E03,b=0.3,c=1E04))
canada.logistic.pred <- data.frame(dateRep=canadadata$dateRep.x, days.since.100=as.numeric(canadadata$days.since.100), cumsum.cases=fitted(canada.logistic))

long.df <- data.frame(days.since.100=0:60)
canada.logistic.pred.long <-data.frame(dateRep=seq(min(canadadata$dateRep.x),min(canadadata$dateRep.x)+60,1), days.since.100=0:60, cumsum.cases=my.logistic(coef(canada.logistic)[1], coef(canada.logistic)[2], coef(canada.logistic)[3], ti=long.df$days.since.100))
...
ggplot(canadadata, aes(dateRep.x, cumsum.cases)) +
  ...
  xlim(min(canadadata$dateRep.x),min(canadadata$dateRep.x)+60) +
  guides(color=guide_legend("Model type")) +
  ggtitle("Canada") 
...
italydata <- df2 %>%
  ...
  select(countriesAndTerritories,dateRep.x,cumsum.cases,cases,days.since.100)
italy.lm <- lm(log(cumsum.cases)~dateRep.x, data=italydata)
italy.lm.pred <- data.frame(dateRep.x=italydata$dateRep, cumsum.cases=exp(fitted(italy.lm)))

italy.logistic <- nls(cumsum.cases ~ my.logistic(a,b,c,as.numeric(days.since.100)), data=italydata, start=list(a=5E03,b=1.1,c=1.5E05))
italy.logistic.pred <- data.frame(dateRep=italydata$dateRep.x, days.since.100=as.numeric(italydata$days.since.100), cumsum.cases=fitted(italy.logistic))


ggplot(italydata, aes(dateRep.x, cumsum.cases)) +
  geom_line(aes(color="data")) +
  geom_point(aes(color="data")) +
  geom_line(data=italy.lm.pred, aes(dateRep.x, cumsum.cases, color="exponential model"), linetype=2) +
...
oo <- order(italydata$dateRep.x)
italydata <- italydata[oo,] # order by date

days <- seq(from=range(italydata$dateRep.x)[1]+2,to=range(italydata$dateRep.x)[2],1)
for(d in days){
  l1 <- which(italydata$dateRep.x==d)
...
ggplot(italydata, aes(dateRep.x, R.3day.avg)) +
  geom_line() + 
  geom_hline(yintercept = 1, lty=2) +
  ylim(0,1.8) +
