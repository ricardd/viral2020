...
```{r}
my.logistic.fct <- function(a=90, b=0.1, c=1E03, ti){
  out <- c / (1 + (a*exp(-b*ti)))
  return(out)
}
...
country.fct <- function(g.id){
  ...
  my.logistic <- nls(cumsum.cases ~ my.logistic.fct(a,b,c,as.numeric(days.since.100)), data=this.df, start=list(a=a.init,b=0.4,c=c.init))
...
italy.logistic <- nls(cumsum.cases ~ my.logistic(a,b,c,as.numeric(days.since.100)), data=italydata, start=list(a=5E03,b=1.2,c=1.5E05))
