...
country.deaths.fct <- function(g.id){
  ...
  logistic.pred <- data.frame(dateRep=this.df$dateRep, days.since.100=as.numeric(this.df$days.since.100), cumsum.deaths=fitted(my.logistic))
  ## inflection point
...
sk.logistic <- nls(cumsum.cases ~ my.logistic(a,b,c,as.numeric(days.since.100.cases)), data=southkoreadata, start=list(a=5E03,b=1.2,c=1E04))
...
us.logistic <- nls(cumsum.cases ~ my.logistic(a,b,c,as.numeric(days.since.100.cases)), data=usdata, start=list(a=1E03,b=0.28,c=1E06))
...
us.logistic.pred.long <-data.frame(dateRep=seq(min(usdata$dateRep),min(usdata$dateRep)+60,1), days.since.100.cases=0:60, cumsum.cases=my.logistic(coef(us.logistic)[1], coef(us.logistic)[2], coef(us.logistic)[3], ti=long.df$days.since.100.cases))
...
canada.logistic <- nls(cumsum.cases ~ my.logistic(a,b,c,as.numeric(days.since.100.cases)), data=canadadata, start=list(a=1E03,b=0.3,c=1E04))
...
long.df <- data.frame(days.since.100.cases=0:60)
canada.logistic.pred.long <-data.frame(dateRep=seq(min(canadadata$dateRep),min(canadadata$dateRep)+60,1), days.since.100.cases=0:60, cumsum.cases=my.logistic(coef(canada.logistic)[1], coef(canada.logistic)[2], coef(canada.logistic)[3], ti=long.df$days.since.100.cases))
...
italy.logistic <- nls(cumsum.cases ~ my.logistic(a,b,c,as.numeric(days.since.100.cases)), data=italydata, start=list(a=5E03,b=0.7,c=2E05))
